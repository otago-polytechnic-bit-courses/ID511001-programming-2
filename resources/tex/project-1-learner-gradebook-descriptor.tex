...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{lastName}
        \item A \textbf{List} of \textbf{AssessmentMarks} objects called \textbf{prog1AssessmentMarks}
        \item A \textbf{List} of \textbf{AssessmentMarks} objects called \textbf{prog2AssessmentMarks}
    \end{itemize}
    \textbf{Note:} A \textbf{Learner} has no behaviours.

    \item An \textbf{AssessmentMarks} object \textbf{must} have the following fields:
    \begin{itemize}
        \item A \textbf{List} of \textbf{int} called \textbf{assessmentMarks}
    \end{itemize}

    \item An \textbf{AssessmentMarks} object has several behaviours such as getting all marks, all grades, the highest mark(s), the lowest mark(s), the fail mark(s), the average mark and the average grade.
    ...
    \item The application \textbf{must} display the following menu:
    \begin{itemize}
        ...
        \item 0. Exit
    \end{itemize}
    \textbf{Note:} You will need to create individual methods to achieve this functionality.

    \item When the user selects \textbf{1. Display all marks}, the application \textbf{must} display all marks for all learners. The marks \textbf{must} be displayed as follows:

    % insert screenshot

    \item When the user selects \textbf{2. Display all grades}, the application \textbf{must} display all grades for all learners. The grades \textbf{must} be displayed as follows:

    % insert screenshot

    \item When the user selects \textbf{3. Display highest, lowest and fail marks}, the application \textbf{must} display the highest, lowest and fail marks for all learners. The marks \textbf{must} be displayed as follows:

    % insert screenshot

    \item When the user selects \textbf{4. Display average marks}, the application \textbf{must} display the average marks for all learners. The marks \textbf{must} be displayed as follows:

    % insert screenshot

    \item When the user selects \textbf{5. Display average grades}, the application \textbf{must} display the average grades for all learners. The grades \textbf{must} be displayed as follows:

    % insert screenshot

    \item When the user selects \textbf{6. Add a learner}, the application \textbf{must} prompt the user to enter the following information:
    ...
    \textbf{Note:} An \textbf{assessment mark} \textbf{must} be between \textbf{0} and \textbf{100}. If an \textbf{assessment mark} is invalid, an error message \textbf{must} be displayed. Also, the \textbf{id} of the learner \textbf{must} be generated automatically. However, the \textbf{id} \textbf{must} be unique. 

    % insert screenshot

    \item When the user selects \textbf{7. Remove a learner}, the application \textbf{must} prompt the user to enter the \textbf{id} of the learner to be removed. If the learner is found, the learner \textbf{must} be removed from the \textbf{List} of \textbf{Learner} objects. If the learner is not found, an error message \textbf{must} be displayed.

    % insert screenshot

    \item When the user selects \textbf{0. Exit}, the application \textbf{must} exit.
